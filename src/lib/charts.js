...
export function renderConfidenceChart(ctx, values, stats) {
  if (!ctx) return null;
  const dark = isDark();
        ...
        tooltip: {
          callbacks: {
            label: (context) => {
              ...
              const s = stats[confKey] || { total: 0, correct: 0 };
              return [
...
export function renderPerformanceChart(ctx, data) {
  if (!ctx) return null;
  const dark = isDark();
  const textColor = dark ? '#9ca3af' : '#6b7280';
  const gridColor = dark ? 'rgba(255, 255, 255, 0.05)' : 'rgba(0, 0, 0, 0.05)';
  const tooltipBg = dark ? 'rgba(30, 41, 59, 0.95)' : 'rgba(255, 255, 255, 0.95)';
  const tooltipText = dark ? '#f3f4f6' : '#1f2937';
  ...
  const labels = chartData.map((item) => {
    ...
    if (item.timestamp && item.timestamp.seconds) {
      return new Date(item.timestamp.seconds * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    }
    if (item.timestamp instanceof Date) {
      return item.timestamp.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    }
    return 'Recently';
  });
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ...
          ticks: { color: textColor, font: { size: 11 }, stepSize: 20, callback: (value) => value + '%' },
        },
      },
    },
  });
