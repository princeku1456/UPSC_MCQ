...
function handleGeneratePractice() {
    ...
    const limit = parseInt(document.getElementById("practice-limit-select").value);
...
function setupPracticeLayout() {
    document.getElementById("quiz-content").innerHTML = `
        <h4 class="text-center mb-4 fw-bold text-info">${practiceChapter}</h4>
...
function renderPracticeQuestion() {
    ...
    q.options.forEach((opt, idx) => {
        ...
        label.innerHTML = `
            <input type="radio" name="pQ" value="${idx}" ${isSelected ? "checked" : ""} ${practiceSubmitted ? "disabled" : ""}>
            <span>${opt}</span>
        `;
...
function updatePracticeNavHighlights() {
    document.querySelectorAll("#practice-nav-container .nav-item").forEach((item, i) => {
            ...
            <div class="row g-2 mb-3">
                <div class="col-12 col-md-4">
                    <div class="p-2 bg-primary text-white rounded shadow-sm">
                        ...
                        <h3 class="fw-bold m-0">${score.toFixed(2)} <span class="fs-6 text-white-50">/ ${totalPossibleMarks}</span></h3>
                    </div>
                </div>
